import React, { useState, useEffect } from "react";
import { Route, Link, Routes } from "react-router-dom";
import "./Navbar1.css";
import { FaBars } from "react-icons/fa";
import { GrFormClose } from "react-icons/gr";

import About from "../About/About";
import Company from "../About/Company";
import Solution from "../Solution/Solution";
import Login from "../Login/Login";

function Navbar() {
  const [isOpen, setOpen] = useState(false);

  const toggleModal = () => {
    setOpen(!isOpen);
  };

  const [activeSublist, setActiveSublist] = useState([]);
  const [activeItem, setActiveItem] = useState("");

  const handleMouseOver = (item) => {
    setActiveItem(item.label);
    setActiveSublist(item.sublist);
  };

  const handleMouseOut = () => {
    setActiveItem("");
    setActiveSublist([]);
  };

  const closeMobileMenu = () => {
    setOpen(false);
  };

  const primaryList = [
    {
      name: "About us",
      to: "/about",
      sublist: [
        { name: "our company", to: "/about/company" },
        { name: "our business", to: "/about/business" },
        { name: "our commitment", to: "/about/commitment" },
      ],
    },
    {
      name: "Our Solutions",
      to: "/solution",
      sublist: [
        { name: "Wealth Planning", to: "/solution/wealth-planing" },
        { name: "Investing", to: "/solution/investing" },
        { name: "Financing", to: "/solution/financing" },
        { name: "Additional services", to: "/solution/additional-services" },
      ],
    },
  ];

  return (
    <div>
      <nav className="transparent other1">
        <div className="inner">
          <div className="left">
            <li className="left-list">
              <div className="globe-int">
                <div className="globe-icon navy">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                    <g>
                      <g>
                        <path d="M14.181 14.043h3.367a8.693 8.693 0 0 1-5.211 3.847c.785-.87 1.426-2.208 1.844-3.847zm-11.382 0h3.367c.418 1.64 1.059 2.976 1.844 3.847a8.693 8.693 0 0 1-5.211-3.847zM8.009 1.11c-.784.87-1.425 2.208-1.843 3.847H2.798A8.693 8.693 0 0 1 8.01 1.11zm6.172 3.847c-.418-1.64-1.06-2.976-1.845-3.847a8.69 8.69 0 0 1 5.212 3.847zm.53 4.14a19.106 19.106 0 0 0-.343-3.314h3.63a8.646 8.646 0 0 1 .828 3.314zm-1.197-3.314a18.11 18.11 0 0 1 .367 3.314H6.465a18.18 18.18 0 0 1 .368-3.314zM10.168.826c1.305 0 2.475 1.675 3.138 4.13H7.031c.663-2.455 1.833-4.13 3.137-4.13zm-7.82 4.957H5.98a19.067 19.067 0 0 0-.344 3.314H1.52a8.646 8.646 0 0 1 .828-3.314zm-.828 4.13h4.115c.023 1.177.142 2.294.344 3.314h-3.63a8.646 8.646 0 0 1-.829-3.314zm5.313 3.314a18.113 18.113 0 0 1-.368-3.314h7.417a18.184 18.184 0 0 1-.368 3.314zm3.335 4.947c-1.304 0-2.474-1.675-3.137-4.13h6.275c-.663 2.455-1.833 4.13-3.138 4.13zm7.83-4.947h-3.63c.201-1.02.321-2.137.344-3.314h4.114a8.646 8.646 0 0 1-.828 3.314zm.633-8.045C17.056 2.109 13.857 0 10.173 0 6.488 0 3.29 2.11 1.714 5.185c-.006.012-.014.023-.018.036A9.438 9.438 0 0 0 .673 9.5c0 1.539.37 2.992 1.023 4.279.004.013.012.024.018.036C3.29 16.89 6.488 19 10.174 19c3.683 0 6.882-2.109 8.457-5.182.007-.014.016-.026.02-.04A9.437 9.437 0 0 0 19.674 9.5c0-1.538-.37-2.99-1.021-4.277-.006-.015-.014-.027-.02-.041z"></path>
                      </g>
                    </g>
                  </svg>
                </div>
                <span className="mobile navy"> INT</span>
                <span className="web navy">International</span>
              </div>
              <span className="navy">EN</span>
              <li className="li-contact ">
                <a href="/" className="navy">
                  contact
                </a>
              </li>
            </li>
          </div>
          <div className="middle">
            <div className="logo">
              <a href="/">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 283.7 55.8"
                >
                  <path
                    d="M49.07 42.07V13.73H38.51v1.86h4v15.69a13.34 13.34 0 0 1-2.11 7.55 7.39 7.39 0 0 1-6.27 3.32 4.35 4.35 0 0 1-3.21-1.08c-1.3-1.3-1.47-3.58-1.47-5.41V13.73h-9.84v1.86h3.29v20.32c0 3 .7 5.12 2.13 6.56s3.78 2.21 7 2.21c4.82 0 8.53-2.21 10.51-6.21v5.47h10.88v-1.87ZM68.71 42.09V.01H58.44v1.85h3.72v40.23h-3.72v1.85h14.6v-1.85h-4.33zM88.6 42.09V13.73H78.33v1.85h3.72v26.51h-3.72v1.85h14.6v-1.85H88.6zM126.65 42.07V13.73h-10.58v1.86h4v15.69a13.34 13.34 0 0 1-2.11 7.55 7.36 7.36 0 0 1-6.26 3.32 4.35 4.35 0 0 1-3.21-1.08c-1.31-1.3-1.47-3.58-1.47-5.41V13.73h-9.85v1.86h3.3v20.32c0 3 .69 5.12 2.13 6.56s3.78 2.21 7 2.21c4.82 0 8.53-2.21 10.5-6.21v5.47h10.88v-1.87ZM217.07 32.07c0 3.8-.94 6.59-2.88 8.54-2.28 2.27-5.87 3.33-11.31 3.33h-20v-1.87h4.2V1.86h-4.2V.01h19.49c4.15 0 7.08.91 9 2.78 1.64 1.65 2.44 4 2.44 7.33 0 5.15-3 8.46-9 9.85 6.18.82 12.26 3.01 12.26 12.1Zm-10.19-9a7.81 7.81 0 0 0-5.81-2h-6.66v21h5.66c8.24 0 9.35-5.31 9.35-10.51.04-3.96-.8-6.73-2.51-8.44ZM204.11 3.3a5.66 5.66 0 0 0-4.22-1.49h-5.5v17h5c4.94 0 6.78-2.17 6.78-8-.01-2.31-.22-5.64-2.03-7.46ZM282.07 19.39a15.7 15.7 0 0 0-5.48-.23c-5.47.69-8.16 6.61-8.16 12.38v10.53h4v1.85h-14.54v-1.85h4V15.54h-4.33v-1.85h10.88v10.3c3.19-6.82 6.81-10.49 10.51-10.43a15.53 15.53 0 0 1 4.56.73ZM242.33 26.13v5.64c0 4.13-1.91 10.69-7 10.69a3.08 3.08 0 0 1-2.3-.87c-1-1-1.5-2.92-1.5-6 0-5.34 1.17-6.59 4.7-7.65Zm7.13 15.23c-.61-.61-.88-1.79-.88-3.8V22.07c0-3-.67-5-2.09-6.43-1.78-1.78-4.8-2.64-9.24-2.64-3.29 0-11 .73-11 7.54a2.31 2.31 0 0 0 .6 1.72 3.2 3.2 0 0 0 2.37.7 3.26 3.26 0 0 0 2.44-.74 3.49 3.49 0 0 0 .77-2.59 5 5 0 0 1 1.13-3.65c.78-.78 2-1.13 3.83-1.13a4.25 4.25 0 0 1 3.18 1.07c1.25 1.24 1.42 3.59 1.42 5.72v2.43a70.79 70.79 0 0 1-6.59 1.91l-3.66.86c-5.51 1.31-7.87 4-7.87 8.82 0 3.27.68 5.48 2.15 6.94s3.47 2.07 6.37 2.07c4.44 0 7.16-1.32 9.33-4.5 1.07 3.48 3.52 3.76 6.83 3.76h4.8v-1.86c-2.09 0-3.19 0-3.89-.71ZM155.68 21.78v-6.85c-3.7-1.19-5.92-1.93-9-1.93-4.94 0-9.92 2.86-9.92 9.25 0 5.73 4.85 7.53 9.14 9.12 3.7 1.38 7.2 2.68 7.2 6.25 0 3.38-3.34 5.2-6.48 5.2-4.59 0-7.72-2.88-8.4-8.08h-2.31v8a30.37 30.37 0 0 0 10.59 1.94 13.22 13.22 0 0 0 9.23-3.45 8.2 8.2 0 0 0 2.46-6c0-7.15-5.32-9-10-10.53-3.59-1.21-6.69-2.26-6.69-5.45a3.67 3.67 0 0 1 1-2.72 5.4 5.4 0 0 1 3.86-1.32 7 7 0 0 1 4.54 1.41 9.66 9.66 0 0 1 2.73 5.15ZM85.33 8.23a3.85 3.85 0 1 0-3.86-3.85 3.85 3.85 0 0 0 3.86 3.85ZM231.73 8.23a3.85 3.85 0 1 0-3.85-3.85 3.85 3.85 0 0 0 3.85 3.85ZM243.93 8.23a3.85 3.85 0 1 0-3.86-3.85 3.85 3.85 0 0 0 3.86 3.85ZM.68 0v1.86h4v42.12c-.19 5.55-.88 8.93-3.84 9.78l-.82.21v1.76a16.39 16.39 0 0 0 3.61-.28c5.26-1.08 8.21-5.09 8.33-11.32V0Z"
                    fill="#001489"
                    class="cls-1"
                    data-color="#001489"
                  />
                </svg>
              </a>
            </div>
          </div>
          <div className="right">
            <div className="lock navy">
              <a href="/" className="lock-link">
                <svg
                  width="19px"
                  height="25px"
                  viewBox="0 0 19 25"
                  version="1.1"
                  xmlns="http://www.w3.org/2000/svg"
                  xmlnsxlink="http://www.w3.org/1999/xlink"
                >
                  <g id="05_08_Navigation_Main">
                    <g
                      id="01_Navigation_Main_New_Desktop1366"
                      transform="translate(-1102.000000, -48.000000)"
                    >
                      <g
                        id="Group"
                        transform="translate(1102.000000, 48.000000)"
                      >
                        <path
                          d="M14.3,6 C14.3,3.3490332 12.1509668,1.2 9.5,1.2 C6.8490332,1.2 4.7,3.3490332 4.7,6 L4.7,9.2 L3.5,9.2 L3.5,6 C3.5,2.6862915 6.1862915,0 9.5,0 C12.8137085,0 15.5,2.6862915 15.5,6 L15.5,6 L15.5,9.2 L14.3,9.2 L14.3,6 L14.3,6 Z"
                          id="lock-arch"
                        />
                        <path
                          d="M15.5,6 L15.499,9 L17,9 C18.1045695,9 19,9.8954305 19,11 L19,23 C19,24.1045695 18.1045695,25 17,25 L2,25 C0.8954305,25 0,24.1045695 0,23 L0,11 C0,9.8954305 0.8954305,9 2,9 L14.299,9 L14.3,6 L15.5,6 Z M17,10.2 L2,10.2 C1.59215895,10.2 1.25559842,10.5051881 1.20623314,10.8996497 L1.2,11 L1.2,23 C1.2,23.407841 1.50518815,23.7444016 1.8996497,23.7937669 L2,23.8 L17,23.8 C17.407841,23.8 17.7444016,23.4948119 17.7937669,23.1003503 L17.8,23 L17.8,11 C17.8,10.592159 17.4948119,10.2555984 17.1003503,10.2062331 L17,10.2 Z"
                          id="Combined-Shape"
                        />
                      </g>
                    </g>
                  </g>
                </svg>
              </a>
            </div>
            <div className="search navy">
              <a href="/" className="search-link">
                <span>
                  <svg
                    viewBox="0 0 25 25"
                    width="25"
                    height="25"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlnsxlink="http://www.w3.org/1999/xlink"
                  >
                    <g stroke="none" stroke-width="1" fill-rule="evenodd">
                      <g>
                        <path d="M15.4265331,14.2014302 C15.4835029,14.230695 15.5369674,14.2690073 15.5847592,14.3163669 L24.812371,23.4605259 C25.0625283,23.7084208 25.0625452,24.1103328 24.8124087,24.3582483 L24.4518168,24.8140305 C24.2016509,25.0619752 23.7960129,25.061992 23.5458262,24.8140679 L14.3180638,15.6697596 C14.2527311,15.6050177 14.2044601,15.5297709 14.1732508,15.4495059 C12.6780166,16.6636764 10.7717328,17.3913043 8.6955061,17.3913043 C3.8930191,17.3913043 -2.27373675e-13,13.4982039 -2.27373675e-13,8.69557913 C-2.27373675e-13,3.89317349 3.89309214,0 8.6955061,0 C13.4979688,0 17.3913043,3.89322227 17.3913043,8.69557913 C17.3913043,10.7848609 16.6544881,12.7020297 15.4265331,14.2014302 Z M16.1490683,8.69557913 C16.1490683,4.57930647 12.8119112,1.24224646 8.6955061,1.24224646 C4.57916363,1.24224646 1.24223602,4.57924379 1.24223602,8.69557913 C1.24223602,12.8121336 4.57909059,16.1490579 8.6955061,16.1490579 C12.8119843,16.1490579 16.1490683,12.8120709 16.1490683,8.69557913 Z" />
                      </g>
                    </g>
                  </svg>
                </span>
              </a>
            </div>
            {/* <Hamburger onToggle={toggleModal} color="#ffffff" />             */}
            <div className="menu-icon navy">
              <FaBars className="ham" onClick={toggleModal} />
            </div>
          </div>
        </div>
      </nav>
      {isOpen && (
        <div id="overlay-modal">
          <div id="modal-content">
            <div className="modal-head">
              <div className="modal-middle">
                <div className="modal-service">
                  <ul>
                    <li>
                      <Link to="/e-services">e-service</Link>
                    </li>
                  </ul>
                </div>
                <div className="logo">
                  <a href="/">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 283.7 55.8"
                    >
                      <path
                        d="M49.07 42.07V13.73H38.51v1.86h4v15.69a13.34 13.34 0 0 1-2.11 7.55 7.39 7.39 0 0 1-6.27 3.32 4.35 4.35 0 0 1-3.21-1.08c-1.3-1.3-1.47-3.58-1.47-5.41V13.73h-9.84v1.86h3.29v20.32c0 3 .7 5.12 2.13 6.56s3.78 2.21 7 2.21c4.82 0 8.53-2.21 10.51-6.21v5.47h10.88v-1.87ZM68.71 42.09V.01H58.44v1.85h3.72v40.23h-3.72v1.85h14.6v-1.85h-4.33zM88.6 42.09V13.73H78.33v1.85h3.72v26.51h-3.72v1.85h14.6v-1.85H88.6zM126.65 42.07V13.73h-10.58v1.86h4v15.69a13.34 13.34 0 0 1-2.11 7.55 7.36 7.36 0 0 1-6.26 3.32 4.35 4.35 0 0 1-3.21-1.08c-1.31-1.3-1.47-3.58-1.47-5.41V13.73h-9.85v1.86h3.3v20.32c0 3 .69 5.12 2.13 6.56s3.78 2.21 7 2.21c4.82 0 8.53-2.21 10.5-6.21v5.47h10.88v-1.87ZM217.07 32.07c0 3.8-.94 6.59-2.88 8.54-2.28 2.27-5.87 3.33-11.31 3.33h-20v-1.87h4.2V1.86h-4.2V.01h19.49c4.15 0 7.08.91 9 2.78 1.64 1.65 2.44 4 2.44 7.33 0 5.15-3 8.46-9 9.85 6.18.82 12.26 3.01 12.26 12.1Zm-10.19-9a7.81 7.81 0 0 0-5.81-2h-6.66v21h5.66c8.24 0 9.35-5.31 9.35-10.51.04-3.96-.8-6.73-2.51-8.44ZM204.11 3.3a5.66 5.66 0 0 0-4.22-1.49h-5.5v17h5c4.94 0 6.78-2.17 6.78-8-.01-2.31-.22-5.64-2.03-7.46ZM282.07 19.39a15.7 15.7 0 0 0-5.48-.23c-5.47.69-8.16 6.61-8.16 12.38v10.53h4v1.85h-14.54v-1.85h4V15.54h-4.33v-1.85h10.88v10.3c3.19-6.82 6.81-10.49 10.51-10.43a15.53 15.53 0 0 1 4.56.73ZM242.33 26.13v5.64c0 4.13-1.91 10.69-7 10.69a3.08 3.08 0 0 1-2.3-.87c-1-1-1.5-2.92-1.5-6 0-5.34 1.17-6.59 4.7-7.65Zm7.13 15.23c-.61-.61-.88-1.79-.88-3.8V22.07c0-3-.67-5-2.09-6.43-1.78-1.78-4.8-2.64-9.24-2.64-3.29 0-11 .73-11 7.54a2.31 2.31 0 0 0 .6 1.72 3.2 3.2 0 0 0 2.37.7 3.26 3.26 0 0 0 2.44-.74 3.49 3.49 0 0 0 .77-2.59 5 5 0 0 1 1.13-3.65c.78-.78 2-1.13 3.83-1.13a4.25 4.25 0 0 1 3.18 1.07c1.25 1.24 1.42 3.59 1.42 5.72v2.43a70.79 70.79 0 0 1-6.59 1.91l-3.66.86c-5.51 1.31-7.87 4-7.87 8.82 0 3.27.68 5.48 2.15 6.94s3.47 2.07 6.37 2.07c4.44 0 7.16-1.32 9.33-4.5 1.07 3.48 3.52 3.76 6.83 3.76h4.8v-1.86c-2.09 0-3.19 0-3.89-.71ZM155.68 21.78v-6.85c-3.7-1.19-5.92-1.93-9-1.93-4.94 0-9.92 2.86-9.92 9.25 0 5.73 4.85 7.53 9.14 9.12 3.7 1.38 7.2 2.68 7.2 6.25 0 3.38-3.34 5.2-6.48 5.2-4.59 0-7.72-2.88-8.4-8.08h-2.31v8a30.37 30.37 0 0 0 10.59 1.94 13.22 13.22 0 0 0 9.23-3.45 8.2 8.2 0 0 0 2.46-6c0-7.15-5.32-9-10-10.53-3.59-1.21-6.69-2.26-6.69-5.45a3.67 3.67 0 0 1 1-2.72 5.4 5.4 0 0 1 3.86-1.32 7 7 0 0 1 4.54 1.41 9.66 9.66 0 0 1 2.73 5.15ZM85.33 8.23a3.85 3.85 0 1 0-3.86-3.85 3.85 3.85 0 0 0 3.86 3.85ZM231.73 8.23a3.85 3.85 0 1 0-3.85-3.85 3.85 3.85 0 0 0 3.85 3.85ZM243.93 8.23a3.85 3.85 0 1 0-3.86-3.85 3.85 3.85 0 0 0 3.86 3.85ZM.68 0v1.86h4v42.12c-.19 5.55-.88 8.93-3.84 9.78l-.82.21v1.76a16.39 16.39 0 0 0 3.61-.28c5.26-1.08 8.21-5.09 8.33-11.32V0Z"
                        fill="#ffffff"
                        class="cls-1"
                        data-color="#001489"
                      />
                    </svg>
                  </a>
                </div>
                <div className="close">
                  <GrFormClose className="close-btn" onClick={toggleModal} />{" "}
                </div>
              </div>{" "}
            </div>
            <div className="modal-body">
              <div className="wrapper">
                <div className="primary-wrapper">
                  <ul>
                    {primaryList.map((item, index) => (
                      
                      <li
                        key={index}
                        onClick={closeMobileMenu}
                        onMouseOver={() => handleMouseOver(item)}
                        onMouseOut={handleMouseOut}
                        className={activeItem === item.name ? "active" : ""}
                      >
                        <Link to={item.to}>{item.name}</Link>
                        <div className="mob-nav-div">
                        <hr className="mob-nav" />
                        </div>
                      </li>
                      
                    ))}
                  </ul>
                </div>
                <div
                  className="secondary-wrapper"
                  onMouseOver={() =>
                    handleMouseOver({
                      name: activeItem,
                      sublist: activeSublist,
                    })
                  }
                  onMouseOut={handleMouseOut}
                >
                  <ul>
                    {activeSublist.map((subItem, index) => (
                      <li key={index}>
                        <Link to={subItem.to}>{subItem.name}</Link>
                      </li>
                    ))}
                  </ul>
                </div>
              </div>
              <div className="modal-footer">
                <ul onClick={closeMobileMenu} >
                  <li> 
                    <Link to="/" >contact</Link>
                  </li>
                  <li>
                  <Link to="/" >legal</Link>
                  </li>
                  <li>
                  <Link to="/e-services" >e-services</Link>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      )}

      <Routes>
        <Route path="/about" Component={About} />
        <Route path="/about/company" Component={Company} />
        <Route path="/solution" Component={Solution} />
        <Route path="/e-services" Component={Login} />
      </Routes>
    </div>
  );
}

export default Navbar;
